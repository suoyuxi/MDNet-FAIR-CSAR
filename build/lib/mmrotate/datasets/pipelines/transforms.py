...
class RayleighQuan(object):
    ...
    def __init__(self, SelRatio=7.5, ScaRatio=4.5, random_range=0.1):
        ...
        self.random_factor = np.random.uniform(low=-random_range, high=random_range)
        self.SelRatio = SelRatio + self.random_factor
        self.ScaRatio = ScaRatio + self.random_factor
    ...
    def __call__(self, results):
        ...
        # provide the w*h*3 power img (-1~1)
        height, width = RetData.shape
        img_power = np.zeros((height, width, 3))
        img_power[:,:,0] = RetData
        img_power[:,:,1] = RetData
        img_power[:,:,2] = RetData

        img_real = np.real(SLCData)
        img_imag = np.imag(SLCData)
        

        results['img'] = img_power.astype(np.float32)
        results['img_real'] = img_real.astype(np.float32)
        results['img_imag'] = img_imag.astype(np.float32)
        results['sar_fields'] = ['img_real', 'img_imag']
...
@ROTATED_PIPELINES.register_module()
class STFTAnalysis(object):

    def __init__(self, window_size=9):

        self.window_size = window_size
        self.windows = self._make_window()
    
    def _make_window(self):
        
        windows = []
        gaussian_window = self._make_gaussian()
        for windowi in range(self.window_size):
            window = np.zeros([1, self.window_size])
            window[0, self.window_size-windowi-1] = 1.0
            windows.append(window*gaussian_window)
        
        return windows
    
    def _make_gaussian(self):

        x = [i-self.window_size//2-1 for i in range(self.window_size)]
        x = np.array(x)
        sigma = self.window_size / 6.0
        gaussian_window = np.exp( -( np.power(x,2) / (2*np.power(sigma,2)) ) )

        return gaussian_window/np.max(gaussian_window)

    def __call__(self, results):

        # get slc data
        img_real = results['img_real']
        img_imag = results['img_imag']
        img_slc = img_real + 1j*img_imag
        # # visualization
        # img_save = self.AGC( np.abs(img_slc) )
        # img_save = (img_save+1.0) * 255.0 / 2.0
        # cv2.imwrite('/workspace/syx/models/TEST/ori.png', img_save.astype(np.uint8))

        # STFT
        img_stft = None
        for window in self.windows:
            img_temp = convolve2d(img_slc, window, mode='same')
            img_temp = np.expand_dims(img_temp, axis=0)
            if img_stft is not None:
                img_stft = np.concatenate([img_stft, img_temp], axis=0)
            else:
                img_stft = img_temp
        img_stft = np.abs( np.fft.fft(img_stft, axis=0) )

        for bandi in range(self.window_size):
            img_stft[bandi,:,:] = self.AGC(img_stft[bandi,:,:])
        #     # visualization
        #     img_save = (img_stft+1.0) * 255.0 / 2.0
        #     img_save = img_save[bandi,:,:]
        #     cv2.imwrite('/workspace/syx/models/TEST/'+str(bandi)+'.png', img_save.astype(np.uint8))
        # exit()

        results['img_stft'] = img_stft.astype(np.float32)
        sar_fields = results['sar_fields']
        sar_fields.append('img_stft')
        results['sar_fields'] = sar_fields

        return results

    def AGC(self, src_img, ratio=0.01):
